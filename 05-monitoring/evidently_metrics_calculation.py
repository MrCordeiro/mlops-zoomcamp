...
def prep_db():
    with psycopg.connect(CONNECTION_STRING, autocommit=True) as conn:
        ...
        with psycopg.connect(f"{CONNECTION_STRING} dbname=test") as conn:
            conn.execute(create_table_statement)


@task
def calculate_metrics_postgresql(curr, i):
    """Calculate metrics from Evidently report for a given day for the target
    columns"""
...
def batch_monitoring_backfill():
    """Flow for backfilling monitoring data"""
    prep_db()
    last_send = datetime.datetime.now() - datetime.timedelta(seconds=10)
    with psycopg.connect(
        f"{CONNECTION_STRING} dbname=test",
        autocommit=True,
    ) as conn:
        for i in range(0, 27):
